...
// Type-only imports: pull the Context/Events declaration merges.
...
export interface UsageLedgerSettings {
   ...
   */
  customPrices: Record<string, { inputCacheHit: number; inputMiss: number; output: number }>
}

/** Schemastery schema of the `usage-ledger` namespace section. */
export const UsageLedgerSchema: z<UsageLedgerSettings> = z.object({
...
/** Per-session model route tracked from `request/header` events. */
...
export function apply(ctx: Context, config: Config): void {
  ...
  const ledger = new UsageLedger(
    resolveLedgerPath(config.ledgerPath),
    settings,
    ctx.logger,
  ...
  const pendingStepUsage = new Map<string, Map<string, UsageSample>>()

  const stepKey = (turn: number, step: number): string => `${turn}/${step}`

  /** Price and record one usage sample for a session. */
  const recordUsage = (sessionId: string, time: number, usage: TokenUsage, turn: number, step: number): void => {
  ...
  const foldEvent = (sessionId: string, event: SessionEvent): void => {
    const type = event.type as string
    const data = event.data as Record<string, unknown>
    switch (type) {
      case 'request/header': {
        const headerConfig = (data.header as { config?: { provider?: string; model?: string } } | undefined)?.config
  ...
  const budgetSectionText = (): string | undefined => {
    if (!ledger.hasBudget) return undefined
    const status = ledger.budgetStatus(Date.now())
    const parts: string[] = []
    if (status.daily.exceeded) parts.push(`今日费用 ${formatCny(status.daily.cost)} 已超出预算 ${formatCny(status.daily.budget)}`)
    else if (status.daily.warn) parts.push(`今日费用 ${formatCny(status.daily.cost)} 已达预算 ${formatCny(status.daily.budget)} 的 ${(status.daily.ratio * 100).toFixed(0)}%`)
    if (status.total.exceeded) parts.push(`累计费用 ${formatCny(status.total.cost)} 已超出总预算 ${formatCny(status.total.budget)}`)
    else if (status.total.warn) parts.push(`累计费用 ${formatCny(status.total.cost)} 已达总预算 ${formatCny(status.total.budget)} 的 ${(status.total.ratio * 100).toFixed(0)}%`)
  ...
  ctx.on('llm/stream', (options: { sessionId?: unknown }, next: () => AsyncIterable<StreamChunk>) => {
  ...
  for (const session of ctx.sessions.list()) foldSession(session)

  // ---- usage_report tool ----
  const renderReport = (view: string, limit: number, currentSessionId?: string): string => {
    const now = Date.now()
    ...
    if (view === 'all' || view === 'budget') {
      ...
      lines.push('')
      lines.push('## 预算状态')
      const describe = (label: string, state: { cost: number; budget: number; ratio: number; exceeded: boolean; warn: boolean }): string => {
        if (state.budget <= 0) return `- ${label}: 未设置预算，已花费 ${formatCny(state.cost)}`
      ...
      if (status.session !== undefined) lines.push(describe('当前会话', status.session))
      lines.push('')
    ...
    if (view === 'all' || view === 'sessions') {
      ...
      if (rows.length === 0) {
        ...
        if (rows.length > top.length) lines.push(`（另有 ${rows.length - top.length} 个会话未列出）`)
      }
    }
  ...
  ctx.inject(['webServer'], (webCtx) => {
    const jsonResponse = (res: ServerResponse, body: unknown): void => {
  ...
  const stopWatch = scope.watch((next) => {
    ...
    syncPromptSection()
    if (next.refreshIntervalMin !== prev.refreshIntervalMin) startRefreshLoop()
    if (next.pricingUrl !== prev.pricingUrl) {
      prices.setUrl(next.pricingUrl)
      void prices.refresh()
    }
    if (next.customPrices !== prev.customPrices) prices.setOverrides(next.customPrices)
  })
