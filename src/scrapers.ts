...
export function parseRawTables(html: string): RawCell[][][] {
...
export function toGrid(rows: RawCell[][]): string[][] {
  const grid: string[][] = []
  ...
  for (const row of rows) {
    ...
    while (index < row.length || carry.has(col)) {
      ...
      if (carried !== undefined) {
        ...
        carried.left -= 1
        if (carried.left <= 0) carry.delete(col)
        col += 1
        continue
      }
      ...
      out.push(cell.text)
      if (cell.rowspan > 1) carry.set(col, { text: cell.text, left: cell.rowspan - 1 })
      col += 1
 ...
 */
export function parsePriceCell(text: string): number | undefined {
  const t = text.replace(/,/g, '')
  if (/免费|free/i.test(t)) return 0
  if (/[$＄]|usd|美元/i.test(t)) return undefined
  const match = t.match(/(\d+(?:\.\d+)?)/)
  if (match === null || match[1] === undefined) return undefined
  let value = Number(match[1])
  if (!Number.isFinite(value)) return undefined
  if (/每千|\/千|千\s*tokens|\/1k|per 1k/i.test(t)) value *= 1000
  return value
}

/** Classify one header/label text into a price kind. */
function kindOf(text: string): keyof ModelPrice | undefined {
  const t = text.toLowerCase()
  const miss = t.includes('未命中') || /miss/.test(t)
  if ((t.includes('缓存命中') || t.includes('命中缓存') || t.includes('cache hit')) && !miss) return 'inputCacheHit'
  if (t.includes('输入') || t.includes('input')) return 'inputMiss'
  if (t.includes('输出') || t.includes('output')) return 'output'
  return undefined
}
...
function parseRowLayout(grid: string[][], idRe: RegExp, table: PriceTable): void {
  for (let h = 0; h < Math.min(grid.length, 4); h += 1) {
    const header = grid[h]
    if (header === undefined) continue
    const kindCols = new Map<number, keyof ModelPrice>()
    header.forEach((text, col) => {
      ...
      if (!isPriceHeader(text)) return
      const kind = kindOf(text)
      if (kind !== undefined && !kindCols.has(col)) kindCols.set(col, kind)
    })
    const kinds = [...kindCols.values()]
    if (!kinds.includes('inputMiss') || !kinds.includes('output')) continue
    let modelCol = header.findIndex(text => /模型|model/i.test(text))
    if (modelCol < 0) modelCol = 0
    for (const row of grid.slice(h + 1)) {
      const match = (row[modelCol] ?? '').match(idRe)
      if (match === null || match[1] === undefined) continue
      const model = match[1].toLowerCase()
      if (table[model] !== undefined) continue // keep the first (lowest) tier
      let inputMiss: number | undefined
      let output: number | undefined
      let cacheHit: number | undefined
      for (const [col, kind] of kindCols) {
        const value = parsePriceCell(row[col] ?? '')
        if (value === undefined) continue
        if (kind === 'inputMiss') inputMiss ??= value
        else if (kind === 'output') output ??= value
        else cacheHit ??= value
      }
      if (inputMiss === undefined || output === undefined) continue
...
function parseColumnLayout(grid: string[][], idRe: RegExp, table: PriceTable): void {
  const header = grid[0]
  if (header === undefined) return
  const modelCols: Array<[number, string]> = []
  header.forEach((text, col) => {
    const match = text.match(idRe)
    if (match !== null && match[1] !== undefined) modelCols.push([col, match[1].toLowerCase()])
  })
  if (modelCols.length === 0) return
  for (const row of grid.slice(1)) {
    ...
    for (const [col, model] of modelCols) {
      ...
      if (existing[kind] === 0) existing[kind] = value
 ...
 */
export function parseVendorSheet(html: string, vendorId: string): PriceTable {
 ...
 */
export function parseErnieSheet(html: string): PriceTable {
  const table: PriceTable = {}
  for (const grid of parseRawTables(html).map(toGrid)) {
    const header = grid[0]
    if (header === undefined) continue
    const find = (re: RegExp): number => header.findIndex(text => re.test(text))
    const modelCol = find(/版本名称/)
    const kindCol = find(/子项/)
    const priceCol = find(/在线推理/)
    const unitCol = find(/单位/)
    if (modelCol < 0 || kindCol < 0 || priceCol < 0) continue
    for (const row of grid.slice(1)) {
      const unit = unitCol >= 0 ? (row[unitCol] ?? '') : ''
      if (!/tokens/i.test(unit)) continue // skip per-image/per-second/GB rows
      const scale = /千/.test(unit) ? 1000 : 1
      const kindText = row[kindCol] ?? ''
      let kind: keyof ModelPrice | undefined
      // 官方表格中缓存价写作"缓存命中"或"命中缓存"两种词序，都要识别。
      if (kindText.includes('缓存命中') || kindText.includes('命中缓存')) kind = 'inputCacheHit'
      else if (kindText.includes('输出')) kind = 'output'
      else if (kindText.includes('输入')) kind = 'inputMiss'
      if (kind === undefined) continue
      ...
      for (const rawId of (row[modelCol] ?? '').split(/\s+/)) {
        ...
        if (!/^ernie[\w.-]*$/.test(model)) continue
        const existing = table[model] ?? { inputCacheHit: 0, inputMiss: 0, output: 0 }
        if (existing[kind] === 0) existing[kind] = value * scale
        table[model] = existing
      }
    }
  }
 ...
 */
export function parseZhipuBundleSheet(js: string): PriceTable {
  const table: PriceTable = {}
 ...
 */
export function parseZhipuLegacySheet(jsonText: string): PriceTable {
  const table: PriceTable = {}
  let slots: Array<{ operationId?: string; content?: string }> = []
  try {
    slots = (JSON.parse(jsonText) as { data?: typeof slots }).data ?? []
  }
  catch {
    return table
  }
  for (const slot of slots) {
    if (slot.operationId !== '1122' && slot.operationId !== '1123') continue
    let content: { list?: Array<{ fieldList?: Array<{ code: string; label?: string }>; modelList?: Array<Record<string, string>> }> }
    try {
    ...
    for (const cat of content.list ?? []) {
      ...
      if (modelCode === undefined || priceCode === undefined) continue
      for (const row of cat.modelList ?? []) {
        const model = (row[modelCode] ?? '').trim().toLowerCase()
        if (!/^glm[\w.-]*$/.test(model)) continue
        const cell = row[priceCode] ?? ''
        if (!/免费|百万\s*tokens/i.test(cell)) continue // skip 元/张、元/万字符 etc.
        const value = parsePriceCell(cell)
        if (value === undefined || table[model] !== undefined) continue
 ...
 */
export function parseDoubaoSheet(markdown: string): PriceTable {
  const table: PriceTable = {}
  const clean = (t: string): string => t.replace(/<br\s*\/?>/gi, ' ').replace(/\\/g, '').replace(/\s+/g, ' ').trim()
  const splitRow = (row: string): string[] => row.replace(/^\|/, '').replace(/\|$/, '').split('|').map(clean)
  ...
  while (i < lines.length) {
    ...
    for (const rowText of block.slice(2)) { // skip header + separator rows
      ...
      if (!model.startsWith('doubao') || model.includes('seedance')) continue
      if (table[model] !== undefined) continue
 ...
 */
export function parseKimiSheet(rscText: string): PriceTable {
  const table: PriceTable = {}
  const blockRe = /columns:\[([\s\S]*?)\],rows:\[\[([\s\S]*?)\]\]/g
  let match: RegExpExecArray | null
  while ((match = blockRe.exec(rscText)) !== null) {
    const titles = [...(match[1] ?? '').matchAll(/title:`([^`]*)`/g)].map(m => m[1] ?? '')
    const kindByCol = new Map<number, keyof ModelPrice>()
    titles.forEach((title, col) => {
      if (title.includes('缓存命中')) kindByCol.set(col, 'inputCacheHit')
      else if (title.includes('缓存未命中') || (title.includes('输入') && !title.includes('缓存'))) kindByCol.set(col, 'inputMiss')
      else if (title.includes('输出')) kindByCol.set(col, 'output')
    })
    ...
    for (const rowText of (match[2] ?? '').split('],[')) {
      ...
      if (!/^(kimi|moonshot)[\w.-]*$/.test(model)) continue
      const price: ModelPrice = { inputCacheHit: 0, inputMiss: 0, output: 0 }
      let priced = false
      ...
      if (priced && (price.inputMiss > 0 || price.output > 0)) table[model] = price
    }
  }
  return table
}
